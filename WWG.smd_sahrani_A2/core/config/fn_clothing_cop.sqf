...
//Shop Title Name
ctrlSetText[3103,"Sahrani Police Department Shop"];
...
switch (_filter) do
{
	...
	case 0:
	{
		...
		{
			_ret pushBack ["A3L_Police_Uniform","Police Uniform",500];
		};
		if(__GETC__(life_coplevel) > 3) then
		{
			_ret pushBack ["A3L_Sheriff_Uniform","Sheriff Uniform",500];
	...
	case 1:
	{
		...
		if(__GETC__(life_coplevel) > 1) then
		{
			...
			_ret pushBack ["A3L_policehelmet","Police Helmet",120];
		};
		
		if(__GETC__(life_coplevel) > 3) then
		{
			_ret pushBack ["A3L_sargehat","Sarge Hat",200];
		};
	};
